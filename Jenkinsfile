...
pipeline {
    ...
    stages {
        ...
        stage('Upload Image to Artifactory') {
            steps {
                sh"docker login -u bldcddbuild.co -p C@passw0rd1 docker-release-candidate-local.artifactory-lvn.broadcom.net"
                sh "docker image build -t docker-release-candidate-local.artifactory-lvn.broadcom.net/com/ca/cdd/dummy-app:$env.BUILD_NUMBER ."
                sh "docker push docker-release-candidate-local.artifactory-lvn.broadcom.net/com/ca/cdd/dummy-app:$env.BUILD_NUMBER"
                sh "docker image build -t docker-release-candidate-local.artifactory-lvn.broadcom.net/com/ca/cdd/dummy-app:latest ."
                sh "docker push docker-release-candidate-local.artifactory-lvn.broadcom.net/com/ca/cdd/dummy-app:latest"
            }
        }
